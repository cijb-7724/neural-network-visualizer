/*
kaggle titanic
Pclass, Sex, FareからAgeを予測し，
そのモデルを用いてAgeの欠損値を補完する
Pclass, Sex, Age, Fareから生死を予測する
*/
...
void data_form(vvd &x, vvd &t, vector<passenger_t> &psg);
//FUNCTION
...
vvd calc_r_mse(vvd &x, vvd &t);
vvd calc_r_identity(vvd &a);
...
int main() {
    ...
    data_form(data_x, data_t, passengers);

    cout << "size is ";
    cout << passengers.size() << endl;
    cout << data_x.size() << ' ' << data_x[0].size() << endl;
    cout << data_t.size() << ' ' << data_t[0].size() << endl;
    ...
    for (int i=0; i<loop; ++i) {
        ...
        if (i % show_interval == 0) {
            ...
            cout << i << "mse ";
            cout << hm_mse(nn[depth-1].x, data_t) << endl;
            if (i % (show_interval*10) == 0) {
                // matrix_show(nn[depth-1].x);
                // matrix_show(t0);
            }
        }
    ...
    cout << "mse ";
    cout << hm_mse(nn[depth-1].x, train_t) << endl;
    for (int i=0; i<40; ++i) cout << "=";
    cout << endl;

    cout << "head show" << endl;
    for (int i=0; i<10; ++i) {
        cout << nn[depth-1].x[i][0] << ' ';
    }cout << endl;
    ...
    cout << "mse ";
    cout << hm_mse(nn[depth-1].x, valid_t) << endl;
    for (int i=0; i<40; ++i) cout << "=";
    cout << endl;
