...
void normalization(vvd &x, vvd &t, vector<passenger_t> &psg) {
    ...
    */
    vd vec_Pclass, vec_Sex, vec_Age, vec_SibSp, vec_Parch, vec_Fare, vec_Embarked;
    for (int i=0; i<n; ++i) {
        ...
        if (psg[i].Embarked == 'S') vec_Embarked.push_back(1);
        else if (psg[i].Embarked == 'C') vec_Embarked.push_back(2.0/3);
        else vec_Embarked.push_back(1.0/3);
    }
    double mx_Pclass, mn_Pclass, mx_Age, mn_Age, mx_SibSp, mn_SibSp;
    double mx_Parch, mn_Parch, mx_Fare, mn_Fare, mx_Embarked, mn_Embarked;
    ...
    mx_Embarked = *max_element(vec_Embarked.begin(), vec_Embarked.end());
    mn_Embarked = *min_element(vec_Embarked.begin(), vec_Embarked.end());
    ...
    for (int i=0; i<n; ++i) {
        ...
        vec_Embarked[i] = (vec_Embarked[i] - mn_Embarked) / (mx_Embarked - mn_Embarked);
    }
    for (int i=0; i<n; ++i) {
        vec_Pclass[i] = 2 * vec_Parch[i] - 1;
        vec_Age[i] = 2 * vec_Age[i] - 1;
        vec_SibSp[i] = 2 * vec_SibSp[i] - 1;
        vec_Parch[i] = 2 * vec_Parch[i] - 1;
        vec_Fare[i] = 2 * vec_Fare[i] - 1;
        vec_Embarked[i] = 2 * vec_Embarked[i] - 1;
    }
    ...
    for (int i=0; i<n; ++i) {
        ...
        //訓練インスタンス
        x.push_back({vec_Pclass[i], vec_Sex[i], vec_Age[i], vec_SibSp[i], vec_Parch[i], vec_Fare[i], vec_Embarked[i]});
    }
}
